...
local icons = require('icons.icon')
...
local function create_title(process_name, base_title, max_width, inset)
    ...
    if base_title == 'Debug' then
        title = icons.GLYPH_DEBUG .. ' DEBUG'
        inset = inset - 2
    end

    if base_title:match('^InputSelector:') ~= nil then
        title = base_title:gsub('InputSelector:', icons.GLYPH_SEARCH)
        inset = inset - 2
    end
...
function Tab:create_cells()
    ...
    self.cells
        :add_segment('scircle_left', icons.GLYPH_SCIRCLE_LEFT)
        :add_segment('admin', ' ' .. icons.GLYPH_ADMIN)
        :add_segment('wsl', ' ' .. icons.GLYPH_LINUX)
        :add_segment('title', ' ', nil, attr(attr.intensity('Bold')))
        :add_segment('unseen_output', ' ' .. icons.GLYPH_CIRCLE)
        :add_segment('padding', ' ')
        :add_segment('scircle_right', icons.GLYPH_SCIRCLE_RIGHT)
end
...
function Tab:update_cells(event_opts, is_active, hover)
    ...
    if event_opts.unseen_icon == 'numbered_box' and self.unseen_output then
        self.cells:update_segment_text(
            'unseen_output',
            ' ' .. icons.GLYPH_UNSEEN_NUMBERED_BOX[self.unseen_output_count]
        )
    end
    if event_opts.unseen_icon == 'numbered_circle' and self.unseen_output then
        self.cells:update_segment_text(
            'unseen_output',
            ' ' .. icons.GLYPH_UNSEEN_NUMBERED_CIRCLE[self.unseen_output_count]
        )
    end
